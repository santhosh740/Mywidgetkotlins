...
package nithra.tamil.tamilclock
class ClockView : View {
    ...
    private var hourHandTrucation = 0
    ...
    private fun drawHand(canvas: Canvas, loc: Double, isHour: Boolean) {
        ...
        val handRadius =
            if (isHour) radius - handTruncation - hourHandTrucation else radius - handTruncation
        canvas.drawLine(
            (width / 2).toFloat(),
            (height / 2).toFloat(),
    ...
    private fun drawNumeral(canvas: Canvas) {
        ...
        for (number in numbers) {
            ...
            val y = (height / 2 + Math.sin(angle) * radius - rect.width() / 2).toInt()
